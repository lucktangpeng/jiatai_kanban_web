  ...
  <div class="board">
    ...
    <table class="table" v-if="ListNew && tableData" :style="tb_height? 'width: 100%; height: 0%':''" >
      ...
      <tr v-for="(item, index) in tableData" :key="index">
        <td v-for="(itemsub,indexs) in ListNew" :key="indexs" :style="itemsub === 'percentage'?'width: 100px;':''">
          <span v-if="itemsub !== 'percentage'">{{item[itemsub]}}</span>
          <el-progress v-else :text-inside="true" :stroke-width="26" :percentage="item.percentage" :status="item.percentage > 100 ? 'exception':'success'"></el-progress>
        </td>
      </tr>
    </table>
    </div>
    ...
    <div class="footer">
      ...
      <div  class="chart" style="width: 49.5%;">
        <div ref="chart_2" id="chart_2" class="canvas"></div>
      </div>
      <!-- <div  class="chart" style="width: 21%;">
        <div ref="chart_3" id="chart_3" class="canvas"></div>
      </div> -->
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    mytime () {
      var that = this
      var yuanliaoTimerInt = window.setInterval(() => {
        var yuanliaoTimer = setTimeout(function () {
          // 列表的最大页数
          ...
          if (that.indexs === number) {
            that.indexs = 0
            if (that.yuanliaoStatus) {
              clearTimeout(yuanliaoTimer)
              clearInterval(yuanliaoTimerInt)
              that.$router.push({name: 'tuogong'})
            }
          }
          ...
          if (that.indexs_line === IndexsLine) {
            that.indexs_line = 0
            // 页面刷新到最后一页就进行换页
            // if (this.tuogongStatus) {
            //   this.$router.push('/tuogong')
            // }
          }
          that.indexs++
          that.indexs_zhu++
