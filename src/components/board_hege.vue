  ...
  <div class="board">
    ...
    <table class="table" v-if="ListNew && tableData" :style="tb_height? 'width: 100%; height: 50%':''" >
      ...
      <tr v-for="(item, index) in tableData" :key="index">
        <td v-for="(itemsub,indexs) in ListNew" :key="indexs" :style="itemsub === 'hgl' || itemsub === 'hgl'?'width: 80px;':''" >
          <el-progress v-if="itemsub === 'hgl'" :text-inside="true" :stroke-width="26" :percentage="item.hgl" :status="item.hgl > 100 ? 'exception':'success'"></el-progress>
  ...
  methods: {
    ...
    getdata () {
      ...
      //       Element.percentage = Math.round((Element.qty_fin / Element.qty) * 100)
            ...
            Element.hgl = Element.hgl.split('%')[0]
            Element.hgl = Number(Element.hgl).toFixed(2)
            that.chart_vallist2.push(Element.qty_all)
            that.chart_keylist.push(Element.NAME)
            that.chart_vallist.push(Element.jk_qty)
          })
